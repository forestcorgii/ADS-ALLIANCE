...
#Region "KI Mode Functions/Subs"
    ...
    Public Sub add_Group()
        If MsgBox("Do you want to save as Group??", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
            executeQRY("INSERT INTO " & MNJOBCODE & "_Groups([Group],[Coordinate]) VALUES(" & String.Format("{0},'{1},{2},{3},{4}'", dtGroups.Rows.Count + 1, CURRENTRECORD, Xorigin, Yorigin, zoomFactor) & ")", conClient)
            MsgBox("Saved")
            get_group()
            load_FieldCoord(mdb_Rec - 1)
        End If
    ...
    Public Sub edit_group()
        ...
        If Integer.Parse(tmp) <= dtGroups.Rows.Count Then
            If MsgBox("Do you want to Edit this Group??", MsgBoxStyle.YesNo) = MsgBoxResult.Yes Then
                executeQRY("UPDATE " & MNJOBCODE & "_Groups SET [Coordinate] = '" & String.Format("{0},{1},{2},{3}", CURRENTRECORD, Xorigin, Yorigin, zoomFactor) & "' WHERE [Group] = " & tmp, conClient)
                MsgBox("Saved")
                get_group()
                load_FieldCoord(mdb_Rec - 1)
            End If
