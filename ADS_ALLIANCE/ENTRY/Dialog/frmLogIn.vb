...
Public Class frmLogin
    ...
    Private Sub txtbox_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles txtLocation.KeyDown, txtOperator.KeyDown, cbJobs.KeyDown
        If (e.KeyCode = Keys.Enter Or e.KeyCode = Keys.Down) And UCase(Me.ActiveControl.Name) = "TXTLOCATION" Then
            txtOperator.Focus()
        ElseIf (e.KeyCode = Keys.Enter) And UCase(Me.ActiveControl.Name) = "CBJOBS" Then
            txtLocation.Select()
        ElseIf e.KeyCode = Keys.Up Then
            If UCase(Me.ActiveControl.Name) = "TXTOPERATOR" Then
                e.Handled = True
                txtLocation.Focus()
            ElseIf UCase(Me.ActiveControl.Name) = "TXTLOCATION" Then
                e.Handled = True
                cbJobs.Select()
            End If
        End If
    ...
    Private Sub cbJobs_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cbJobs.SelectedIndexChanged
        If Not EXE = cbJobs.Text Then
            EXE = cbJobs.Text
            My.Settings.EXE = EXE
            My.Settings.Save()

            Dim temp() As String = AppName.Split("-")
            lblInfo.Text = temp(0) & "-" & EXE
